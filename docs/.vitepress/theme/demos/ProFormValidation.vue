...
const model = ref({ username: '', password: '', confirmPassword: '', phone: '', idCard: '' })
...
const columns: ProFormColumn[] = [
  {
    ...
    rule: [
      ...
      { min: 4, max: 16, message: '长度4-16位', trigger: 'blur' },
    ],
    span: 12,
  },
  {
  ...
  },
  {
    ...
    rule: { pattern: /^1[3-9]\d{9}$/, message: '手机号格式不正确', trigger: 'blur' },
    span: 12,
  },
  {
    key: 'idCard',
    ...
    rule: { pattern: /^\d{17}[\dX]$/i, message: '身份证格式不正确', trigger: 'blur' },
    span: 24,
  },
]
</script>
